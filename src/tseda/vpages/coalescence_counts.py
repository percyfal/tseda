"""Module to plot coalescence counts"""
...
class PairCoalescenceCounts(View):
    num_time_bins = param.Integer(
        default=100, bounds=(1, None), doc="Number of time bins"
    )
    window_size = param.Integer(
    ...
    def __panel__(self):
        ...
        for k, v in zip(
            pairs,
            tsm.ts.pair_coalescence_counts(
                sample_sets=list(sample_sets.values()), indexes=indexes
            ),
        ):
            x[k] = hv.Histogram(v)

        return pn.pane.HoloViews(
            hv.GridSpace(x, kdims=["pop1", "pop2"]),
    ...
    def sidebar(self):
        return pn.Card(
            ...
            self.param.window_size,
            collapsed=False,
            title="Coalescence counts plotting options",
            header_background=config.SIDEBAR_BACKGROUND,
            active_header_background=config.SIDEBAR_BACKGROUND,
            styles=config.VCARD_STYLE,
        )
...
class CoalescenceCountsPage(View):
    key = "coalescence_counts"
    title = "Coalescence Counts"
    data = param.ClassSelector(class_=PairCoalescenceCounts)

    def __init__(self, **params):
        super().__init__(**params)
        self.data = PairCoalescenceCounts(datastore=self.datastore)

    def __panel__(self):
        return pn.Column(
            pn.pane.Markdown("## Coalescence Counts Mockup"),
            self.data,
        )
